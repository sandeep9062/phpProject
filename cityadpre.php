...
$optp="SELECT period,nxt FROM `wheelperiod`";
...
$opt="SELECT SUM(amount) as total FROM `wheelbetting` WHERE status='pending' AND ans='red'";
$optres=$conn->query($opt);
$sum= mysqli_fetch_assoc($optres);
$red=round($sum['total'],2);

$optg="SELECT SUM(amount) as total FROM `wheelbetting` WHERE status='pending' AND ans='green'";
$optresg=$conn->query($optg);
$sumg= mysqli_fetch_assoc($optresg);
$green=round($sumg['total'],2);

$optv="SELECT SUM(amount) as total FROM `wheelbetting` WHERE status='pending' AND ans='black'";
$optresv=$conn->query($optv);
$sumv= mysqli_fetch_assoc($optresv);
$violet=round($sumv['total'],2);

$opt9="SELECT SUM(amount) as total FROM `wheelbetting` WHERE status='pending' AND ans='blue'";
...
">Jhon Doe</h6>
     ...
     <h2 style="font-size:20px;padding:10px;color:grey;  text-align:center;" onclick="window.location.href='wheeladpre'">Circle</h2>
      <h2 style="font-size:20px;padding:10px;  text-align:center;" onclick="window.location.href='cityadpre'">WheeloCity</h2>
 ...
 <div data-v-309ccc10="" class="recharge">
      ...
      <h1>4---GREEN</h1><br>
        ...
        <form action="citypre" id="pre" method="POST" class="form-signup">
            ...
            <div data-v-309ccc10="" class="input_box"><img data-v-309ccc10=""
                    ...
                    placeholder="Enter a number from 1-4"><span data-v-309ccc10="" class="tips_span">Next perdiction</span></div>
            ...
            <div data-v-309ccc10="" class="input_box_btn"><button data-v-309ccc10="" type="button" onclick="sub()"
                    class="login_btn ripple">Confirm Next Prediction</button></div>
                     ...
                     <div id="copied">Enter a number from 1-4</div>
