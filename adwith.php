...
$name=$row2['name'];
    ...
    $dataRow = $dataRow."<tr><td>$row2[1]</td><td>$row2[type]</td><td>$row2[2]</td><td>$row2[3]</td><td>$name</td><td>$bankname</td><td>$bank</td><td>$ifsc</td><td onclick='window.location.href=$agree;' style='background:green;'>AGREE</td><td onclick='window.location.href=$alink;' style='background:green;'>YES</td><td><input id='$row2[0]' typt='text' value='$row2[note]'></td><td onclick=decline('$row2[2]','$row2[1]','$row2[0]','$row2[type]') style='background:red;'>NO</td></tr>";
}
...
<head>
    ...
    <link rel="apple-touch-icon" href="/img/with.png">
...
tr:nth-child(even) {
  ...
  <script>
       function decline(amount, username, id, type) {
    var note = document.getElementById(id).value;
    if (!note) {
        alert("Enter a reason");
    } else {
        window.location.href = "/wdecline?am=" + amount + "&un=" + username + "&id=" + id + "&note=" + note + "&type=" + type;
    }
}
...
">Jhon Doe</h6>
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="adwith" class="nav-item nav-link active"><i class="fa fa-table me-2"></i>Withdraw Req</a>
